...
@else
	@foreach ( $shared_items as $user )
		<div class="py-4 dark:text-gray-200" x-data={show:true}>
			<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
				<div class="mx-2 rounded-md bg-gray-100 dark:bg-gray-600 overflow-hidden shadow-xl sm:rounded-lg p-5">
					<div class="flex">
						<div class="flex-none">
							<button type="button" @click="show=!show" class="my-auto" onclick="toggleSection('{{ $user->id }}')" id="name{{ $user->id}}">
								<div class="flex">
					...
					@else
						<div id="collapsable_section" x-show="show">
							<div class="flex mx-auto mt-5 px-4 max-h-6">
								<div class="relative w-full rounded-full bg-gradient-to-r from-green-600 via-yellow-300 to-red-600">
									<span class="invisible">.</span>
									<div class="absolute top-0 right-0 w-[{{ $user->claimed_item_percentage() }}%] rounded-r-full bg-gray-200 h-full">
									</div>
									<div class="absolute top-0 left-0 w-full text-center my-auto font-bold">
										{{ 100 - round($user->claimed_item_percentage(), 2) }}% Claimed
									</div>
								</div>
							</div>
